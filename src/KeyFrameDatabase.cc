...
namespace ORB_SLAM3
{
    ...
    {
        std::map<unsigned long int, KeyFrame*> spConnectedKeyFrames = pKF->GetConnectedKeyFrames();
            ...
            for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(), vend = pKF->mBowVec.end(); vit != vend; vit++)
            {
                ...
                for (auto pKFi : lKFs)
                {
                    if (pKFi->GetMap() == pKF->GetMap()) // For consider a loop candidate it a candidate it must be in the same map
                    {
                        ...
                        {
                            pKFi->mnLoopWords = 0;
                            if (!spConnectedKeyFrames.count(pKFi->mnId))
                            {
        ...
        map<unsigned long int, KeyFrame*> spAlreadyAddedKF;
        ...
        for (auto & it : lAccScoreAndMatch)
        {
            ...
            {
                KeyFrame *pKFi = it.second;
                if (!spAlreadyAddedKF.count(pKFi->mnId))
                {
                    vpLoopCandidates.push_back(pKFi);
                    spAlreadyAddedKF[pKFi->mnId] = pKFi;
                }
            }
        }
    ...
    {
        list<KeyFrame *> lKFsSharingWords;
        std::map<unsigned long int, KeyFrame*> spConnectedKF;
        ...
        // Search all keyframes that share a word with current frame
        {
            ...
            for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(), vend = pKF->mBowVec.end(); vit != vend; vit++)
            {
                ...
                for (auto pKFi : lKFs)
                {
                    if (spConnectedKF.find(pKFi->mnId) != spConnectedKF.end())
                    {
                        continue;
                    }
    ...
    {
        list<KeyFrame *> lKFsSharingWords;
        std::map<long unsigned int, KeyFrame*> spConnectedKF;
        ...
        // Search all keyframes that share a word with current frame
        {
            ...
            for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(), vend = pKF->mBowVec.end(); vit != vend; vit++)
            {
                ...
                for (auto pKFi : lKFs)
                {
                    ...
                    {
                        pKFi->mnPlaceRecognitionWords = 0;
                        if (!spConnectedKF.count(pKFi->mnId))
                        {
                            pKFi->mnPlaceRecognitionQuery = pKF->mnId;
                            lKFsSharingWords.push_back(pKFi);
                        }
