...
namespace ORB_SLAM3
{
                                                                                                                          ...
                                                                                                                          mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
    {
        ...
        mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor == MONOCULAR || mSensor == IMU_MONOCULAR,
                                         mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD, settings_, strSequence);
    ...
    Sophus::SE3f System::TrackStereoSemantic(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &imLeftSem, const unordered_map<int, bool> &seg_meta, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename)
    {
        if (mSensor != STEREO && mSensor != IMU_STEREO)
        {
            cerr << "ERROR: you called TrackStereo but input sensor was not set to Stereo nor Stereo-Inertial." << endl;
        ...
        cv::Mat imLeftToFeed, imRightToFeed, imLeftSemToFeed;
        if (settings_ && settings_->needToRectify())
        {
            cv::Mat M1l = settings_->M1l();
            cv::Mat M2l = settings_->M2l();
            ...
            cv::remap(imLeft, imLeftToFeed, M1l, M2l, cv::INTER_LINEAR);
            cv::remap(imLeftSem, imLeftSemToFeed, M1l, M2l, cv::INTER_LINEAR);
            cv::remap(imRight, imRightToFeed, M1r, M2r, cv::INTER_LINEAR);
        }
        else if (settings_ && settings_->needToResize())
        {
            cv::resize(imLeft, imLeftToFeed, settings_->newImSize());
            cv::resize(imLeftSem, imLeftSemToFeed, settings_->newImSize());
            cv::resize(imRight, imRightToFeed, settings_->newImSize());
        }
        else
        {
            imLeftToFeed = imLeft.clone();
            imLeftSemToFeed = imLeftSem.clone();
            imRightToFeed = imRight.clone();
        }
        ...
        // std::cout << "start GrabImageStereo" << std::endl;
        Sophus::SE3f Tcw = mpTracker->GrabImageStereoSemantic(imLeftToFeed, imRightToFeed, imLeftSemToFeed, seg_meta, timestamp, filename);
