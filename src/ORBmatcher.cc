...
namespace ORB_SLAM3
{
    ...
    ORBmatcher::ORBmatcher(float nnratio, bool checkOri, int use_semantic, int use_instance) : mfNNratio(nnratio), mbCheckOrientation(checkOri), mUseSemantic(use_semantic), mUseInstance(use_instance)
    {
    }
            ...
            if (pMP->mbTrackInView)
            {
                ...
                if (!vIndices.empty())
                {
                    ...
                    if (bestDist <= TH_HIGH)
                    {
                        ...
                        if (bestLevel != bestLevel2 || bestDist <= mfNNratio * bestDist2)
                        {
                            pMP->AddSemObservation(F.mvSemanticCls[bestIdx]);
                            pMP->AddInstanceObservation(F.mvInstanceCls[bestIdx]);

                            if (mUseSemantic && (F.mvSemanticCls[bestIdx] != pMP->mvSemanticCls))
                            {
                                continue;
                            }
                            if (mUseInstance && (F.mvInstanceCls[bestIdx] != pMP->mvInstanceCls))
                            {
                                continue;
                            }
            ...
            if (KFit->first == Fit->first)
            {
                ...
                for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++)
                {
                    ...
                    if (bestDist1 <= TH_LOW)
                    {
                        ...
                        {
                            pMP->AddSemObservation(F.mvSemanticCls[bestIdxF]);
                            pMP->AddInstanceObservation(F.mvInstanceCls[bestIdxF]);
                            if (mUseSemantic && (F.mvSemanticCls[bestIdxF] != pMP->mvSemanticCls))
                            {
                                continue;
                            }
                            if (mUseInstance && (F.mvInstanceCls[bestIdxF] != pMP->mvInstanceCls))
                            {
                                continue;
                            }
                                       ...
                                       vector<MapPoint *> &vpMatched, int th, float ratioHamming)
    {
        ...
        for (int iMP = 0, iendMP = vpPoints.size(); iMP < iendMP; iMP++)
        {
            ...
            for (vector<size_t>::const_iterator vit = vIndices.begin(), vend = vIndices.end(); vit != vend; vit++)
            {
                ...
                if (vpMatched[idx])
                    continue;
                // if (mUseSemantic && (pKF->mvSemanticCls[idx] != pMP->mvSemanticCls))
                // {
                //     continue;
                // }
                // if (mUseInstance && (pKF->mvInstanceCls[idx] != pMP->mvInstanceCls))
                // {
                //     continue;
                // }
            ...
            {
                pMP->AddSemObservation(pKF->mvSemanticCls[bestIdx]);
                pMP->AddInstanceObservation(pKF->mvInstanceCls[bestIdx]);
                if (mUseSemantic && (pKF->mvSemanticCls[bestIdx] != pMP->mvSemanticCls))
                {
                    continue;
                }
                if (mUseInstance && (pKF->mvInstanceCls[bestIdx] != pMP->mvInstanceCls))
                {
                    continue;
                }
    ...
    int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
    {
        ...
        for (size_t i1 = 0, iend1 = F1.mvKeysUn.size(); i1 < iend1; i1++)
        {
            ...
            for (vector<size_t>::iterator vit = vIndices2.begin(); vit != vIndices2.end(); vit++)
            {
                ...
                if (mUseSemantic && (F2.mvSemanticCls[i2] != F1.mvSemanticCls[i1]))
                {
                    continue;
                }
                if (mUseInstance && (F2.mvInstanceCls[i2] != F1.mvInstanceCls[i1]))
                {
                    continue;
                }
            ...
            if (f1it->first == f2it->first)
            {
                for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++)
                {
                    ...
                    for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++)
                    {
                        ...
                        if (pMP2->isBad())
                            continue;
                        if (mUseSemantic && (pMP2->mvSemanticCls != pMP1->mvSemanticCls))
                        {
                            continue;
                        }
                        if (mUseInstance && (pMP2->mvInstanceCls != pMP1->mvInstanceCls))
                        {
                            continue;
                        }
            ...
            if (f1it->first == f2it->first)
            {
                for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++)
                {
                    ...
                    for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++)
                    {
                        ...
                        if (mUseSemantic && (pKF2->mvSemanticCls[idx2] != pKF1->mvSemanticCls[idx1]))
                        {
                            continue;
                        }
                        if (mUseInstance && (pKF2->mvInstanceCls[idx2] != pKF1->mvInstanceCls[idx1]))
                        {
                            continue;
                        }
    ...
    int ORBmatcher::Fuse(KeyFrame *pKF, Sophus::Sim3f &Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
    {
        ...
        // Set of MapPoints already found in the KeyFrame
        const map<unsigned long int, MapPoint *> spAlreadyFound = pKF->GetMapPoints();
    ...
    int ORBmatcher::SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<MapPoint *> &vpMatches12, const Sophus::Sim3f &S12, const float th)
    {
        ...
        for (int i1 = 0; i1 < N1; i1++)
        {
            ...
            {
                const size_t idx = *vit;
                if (mUseSemantic && (pKF2->mvSemanticCls[idx] != pMP->mvSemanticCls))
                {
                    continue;
                }
                if (mUseInstance && (pKF2->mvInstanceCls[idx] != pMP->mvInstanceCls))
                {
                    continue;
                }
        ...
        for (int i2 = 0; i2 < N2; i2++)
        {
            ...
            {
                const size_t idx = *vit;
                if (mUseSemantic && (pKF1->mvSemanticCls[idx] != pMP->mvSemanticCls))
                {
                    continue;
                }
                if (mUseInstance && (pKF1->mvInstanceCls[idx] != pMP->mvInstanceCls))
                {
                    continue;
                }
        ...
        for (int i = 0; i < LastFrame.N; i++)
        {
            ...
            if (pMP)
            {
                if (!LastFrame.mvbOutlier[i])
                {
                    ...
                    if (bestDist <= TH_HIGH)
                    {
                        pMP->AddSemObservation(CurrentFrame.mvSemanticCls[bestIdx2]);
                        pMP->AddInstanceObservation(CurrentFrame.mvInstanceCls[bestIdx2]);
                        if (mUseSemantic && (CurrentFrame.mvSemanticCls[bestIdx2] != pMP->mvSemanticCls))
                        {
                            continue;
                        }
                        if (mUseInstance && (CurrentFrame.mvInstanceCls[bestIdx2] != pMP->mvInstanceCls))
                        {
                            continue;
                        }
