...
namespace ORB_SLAM3
{
    ...
    Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase *pKFDB, const string &strSettingPath, const int sensor, Settings *settings, const string &_nameSeq) : mState(NO_IMAGES_YET), mSensor(sensor), mTrackedFr(0), mbStep(false),
                                                                                                                                                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                                                                                                                                                       mpMapDrawer(pMapDrawer), bStepByStep(false), mpAtlas(pAtlas), mpLastKeyFrame(static_cast<KeyFrame *>(nullptr)), mnLastRelocFrameId(0),
    ...
    void Tracking::newParameterLoader(Settings *settings)
    {
        ...
        mLinesNFeatures = settings->linesNFeatures();
        mLinesLsdRefine = settings->linesLsdRefine();
        mLinesLsdScale = settings->linesLsdScale();
        mLinesLevels = settings->linesLevels();
        mLinesScale = settings->linesScale();
        ...
        mpORBextractorLeft = new ORBextractor(nFeatures, fScaleFactor, nLevels, fIniThFAST, fMinThFAST, mDropMoving, mDropDynamic, mUseSemantic, mUseInstance);

        if (mSensor == System::STEREO || mSensor == System::IMU_STEREO)
            mpORBextractorRight = new ORBextractor(nFeatures, fScaleFactor, nLevels, fIniThFAST, fMinThFAST, mDropMoving, mDropDynamic, mUseSemantic, mUseInstance);
    ...
    bool Tracking::ParseIMUParamFile(cv::FileStorage &fSettings)
    {
        ...
        std::cout << endl;
        std::cout << "Left camera to Imu Transform (Tbc): " << endl
             << cvTbc << endl;
    ...
    Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
    {
        ...
        {
            // std::cout << "Image with 3 channels" << endl;
            if (mbRGB)
            {
        ...
        {
            // std::cout << "Image with 4 channels" << endl;
            if (mbRGB)
            {
    ...
    Sophus::SE3f Tracking::GrabImageStereoSemantic(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const cv::Mat &imLeftSem, const unordered_map<int, bool> &seg_meta, const double &timestamp, string filename)
    {
        // std::cout << "GrabImageStereo" << endl;
        ...
        cvtColor(mImSem, mImSem, cv::COLOR_BGR2RGB);
    ...
    void Tracking::TrackDefault()
    {
        ...
        if (!pCurrentMap)
        {
            std::cout << "ERROR: There is not an active map in the atlas" << endl;
        }
        ...
        else
        {
            ...
            if (bOK || mState == RECENTLY_LOST)
            {
                ...
                for (int i = 0; i < mCurrentFrame.N; i++)
                {
                    ...
                    if (pMP)
                    {
                        if (mDropMoving && mCurrentFrame.mvKeysMoving[i])
                        {
                ...
                for (int i = 0; i < mCurrentFrame.N; i++)
                {
                    if (mDropMoving && mCurrentFrame.mvKeysMoving[i] && mCurrentFrame.mvpMapPoints[i])
                    {
                        MapPoint *pMP = mCurrentFrame.mvpMapPoints[i];
                        pMP->SetBadFlag();
                        continue;
    ...
    void Tracking::TrackWithLines()
    {
        ...
        else
        {
            ...
            if (bOK || mState == RECENTLY_LOST)
            {
                ...
                for (int i = 0; i < mCurrentFrame.N_Lines; i++)
                {
                    if (mCurrentFrame.mvpMapLines[i] && mCurrentFrame.mvbOutlierLine[i])
                        mCurrentFrame.mvpMapLines[i] = static_cast<MapLine*>(NULL);
                }
            }
    ...
    void Tracking::StereoInitialization()
    {
        if (mCurrentFrame.N > 500)
        {
                ...
                for (int i = 0; i < mCurrentFrame.N; i++)
                {
                    ...
                    if (z > 0)
                    {
                        ...
                        else
                        {
                            ...
                            auto *pNewMP = new MapPoint(x3D, pKFini, mpAtlas->GetCurrentMap(), mCurrentFrame.mvSemanticCls[i], mCurrentFrame.mvInstanceCls[i]);
            ...
            // std::cout << "Active map: " << mpAtlas->GetCurrentMap()->GetId() << endl;
    ...
    {
        if (mCurrentFrame.N + mCurrentFrame.N_Lines > 500)
        {
            if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            {
                if (!mCurrentFrame.mpImuPreintegrated || !mLastFrame.mpImuPreintegrated)
            ...
            if (!mpCamera2)
            {
                ...
                for (int i = 0; i < mCurrentFrame.N_Lines; i++)
                {
                    std::pair<float, float> depth = mCurrentFrame.mvDepthLine[i];
    ...
    void Tracking::CreateInitialMapMonocular()
    {
        ...
        std::map<unsigned long int, MapPoint *> sMPs;
    ...
    void Tracking::CheckReplacedInLastFrameWithLines()
    {
        ...
        for (int i = 0; i < mLastFrame.N_Lines; i++)
        {
    ...
    bool Tracking::TrackReferenceKeyFrame()
    {
        ...
        ORBmatcher matcher(0.7, true, mUseSemantic, mUseInstance);
        ...
        // std::cout << " TrackReferenceKeyFrame mLastFrame.mTcw:  " << mLastFrame.mTcw << endl;
        for (int i = 0; i < mCurrentFrame.N; i++)
        {
    ...
    bool Tracking::TrackReferenceKeyFrameWithLines()
    {
        ...
        // We perform first an ORB matching with the reference keyframe
        // If enough matches are found we setup a PnP solver
        ORBmatcher orb_matcher(0.7, true, mUseSemantic, mUseInstance);
        vector<MapPoint *> vpMapPointMatches;

        int nmatches_points = orb_matcher.SearchByBoW(mpReferenceKF, mCurrentFrame, vpMapPointMatches);

        // Line matching
        LineMatcher line_matcher(0.9, 3.0, M_PI/8.0);
        vector<MapLine*> vpMapLineMatches;
        int nmatches_lines = line_matcher.match(mpReferenceKF, mCurrentFrame, vpMapLineMatches, true);

        int nmatches = nmatches_points + nmatches_lines;
        ...
        if (mDropMoving) 
        {
            for (int i = 0; i < mCurrentFrame.N; i++)
            {
                if (mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvKeysMoving[i])
                {
                    MapPoint *pMP = mCurrentFrame.mvpMapPoints[i];
                    pMP->SetBadFlag();
                }
    ...
    void Tracking::UpdateLastFrame()
    {
        ...
        for (size_t j = 0; j < vDepthIdx.size(); j++)
        {
            ...
            if (!pMP)
                ...
                bCreateNew = true;
            if (mDropMoving && mLastFrame.mvKeysMoving[i])
            {
                bCreateNew = false;
            }
            ...
            if (bCreateNew)
            {
                ...
                MapPoint *pNewMP = new MapPoint(x3D, mpAtlas->GetCurrentMap(), &mLastFrame, i, mLastFrame.mvSemanticCls[i], mLastFrame.mvInstanceCls[i]);
    ...
    void Tracking::UpdateLastFrameWithLines()
    {
        ...
        vDepthIdx.clear();
        vDepthIdx.reserve(mLastFrame.N_Lines);

        for (int i = 0; i < mLastFrame.N_Lines; i++)
        {
            float sz, ez;
            sz = mLastFrame.mvDepthLine[i].first;
            ez = mLastFrame.mvDepthLine[i].second;
            if (sz > 0 && ez > 0)
            {
                float z = sz > ez ? sz : ez;
                vDepthIdx.push_back(make_pair(z, i));
    ...
    bool Tracking::TrackWithMotionModelWithLines()
    {
        ...
        int nmatches_points = orb_matcher.SearchByProjection(mCurrentFrame, mLastFrame, th, mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
        for (int i = 0; i < mCurrentFrame.N; i++)
        {
            if (mCurrentFrame.mvpMapPoints[i])
            {
        ...
        // If few matches, uses a wider window search
        if (nmatches_points < 20)
        {
            Verbose::PrintMess("Not enough matches, wider window search!!", Verbose::VERBOSITY_NORMAL);
            fill(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(), static_cast<MapPoint *>(NULL));
            nmatches_points = orb_matcher.SearchByProjection(mCurrentFrame, mLastFrame, 2 * th, mSensor == System::MONOCULAR || mSensor == System::IMU_MONOCULAR);
            for (int i = 0; i < mCurrentFrame.N; i++)
            {
                if (mCurrentFrame.mvpMapPoints[i])
                {
            ...
            Verbose::PrintMess("Matches with wider search: " + to_string(nmatches_points), Verbose::VERBOSITY_NORMAL);
        }

        fill(mCurrentFrame.mvpMapLines.begin(), mCurrentFrame.mvpMapLines.end(), static_cast<MapLine *>(NULL));

        int nmatches_lines = line_matcher.SearchByProjection(mCurrentFrame, mLastFrame, true);
        ...
        for (int i = 0; i < mCurrentFrame.N_Lines; i++)
        {
            if (mCurrentFrame.mvpMapLines[i]) 
            {
                if (mCurrentFrame.mvbOutlierLine[i]) 
                {
                    MapLine *pML = mCurrentFrame.mvpMapLines[i];
    ...
    bool Tracking::TrackLocalMapWithLines()
    {
        ...
        mnMatchesInliers = mnMatchesInliersPoints + mnMatchesInliersLines;
    ...
    bool Tracking::NeedNewKeyFrameWithLines()
    {
        ...
        // Check how many "close" points are being tracked and how many could be potentially created.
        int nNonTrackedClosePoints = 0;
        int nTrackedClosePoints = 0;
            ...
            for (int i = 0; i < N; i++)
            {
                if (mCurrentFrame.mvDepth[i] > 0 && mCurrentFrame.mvDepth[i] < mThDepth)
                {
                    if (mCurrentFrame.mvpMapPoints[i] && !mCurrentFrame.mvbOutlier[i])
                        ...
                        nNonTrackedClosePoints++;
                }
            }
        }

        int nNonTrackedCloseLines = 0;
        int nTrackedCloseLines = 0;

        for(int i = 0; i < mCurrentFrame.N_Lines; i++) 
        {
            float sz = mCurrentFrame.mvDepthLine[i].first;
            float ez = mCurrentFrame.mvDepthLine[i].second;
            ...
            if(sz > 0 && ez > 0 && z < mThDepth)
            {
                if(mCurrentFrame.mvpMapLines[i] && !mCurrentFrame.mvbOutlierLine[i])
                    ...
                    nNonTrackedCloseLines++;

            }
        }

        int nTrackedClose = nTrackedClosePoints + nTrackedCloseLines;
        int nNonTrackedClose = nNonTrackedClosePoints + nNonTrackedCloseLines;
        
        bool bNeedToInsertClose;
        bNeedToInsertClose = (nTrackedClose < 140) && (nNonTrackedClose > 100);
        ...
        const bool c1b = ((mCurrentFrame.mnId >= mnLastKeyFrameId + mMinFrames) && bLocalMappingIdle); // mpLocalMapper->KeyframesInQueue() < 2);
        // Condition 1c: tracking is weak
        const bool c1c = mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR && mSensor != System::IMU_STEREO && mSensor != System::IMU_RGBD && (mnMatchesInliersPoints < nRefMatches * 0.25 || bNeedToInsertClose);
        // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
        const bool c2 = (((mnMatchesInliersPoints < nRefMatches * thRefRatio || bNeedToInsertClose)) && mnMatchesInliersPoints > 15);
        ...
        bool c4 = false;
        if ((((mnMatchesInliersPoints < 75) && (mnMatchesInliersPoints > 15)) || mState == RECENTLY_LOST) && (mSensor == System::IMU_MONOCULAR))
            c4 = true;
        else
            c4 = false;
            ...
            if (!vDepthIdx.empty())
            {
                ...
                for (size_t j = 0; j < vDepthIdx.size(); j++)
                {
                    ...
                    if (bCreateNew)
                    {
                        ...
                        MapPoint *pNewMP = new MapPoint(x3D, pKF, mpAtlas->GetCurrentMap(), mCurrentFrame.mvSemanticCls[i], mCurrentFrame.mvInstanceCls[i]);
    ...
    void Tracking::CreateNewKeyFrameWithLines()
    {
        ...
        if (mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR) // TODO check if incluide imu_stereo
        {
            ...
            vDepthIdx.clear();
            vDepthIdx.reserve(mCurrentFrame.N_Lines);
            ...
            if (!vDepthIdx.empty())
            {
                ...
                for (size_t j = 0; j < vDepthIdx.size(); j++)
                {
                    ...
                    if(vDepthIdx[j].first > mThDepth && nLines > maxLines)
                        break;
                }
            }
    ...
    void Tracking::SearchLocalPoints()
    {
        ...
        if (nToMatch > 0)
        {
            ORBmatcher matcher(0.8, mUseSemantic, mUseInstance);
            int th = 1;
            if (mSensor == System::RGBD || mSensor == System::IMU_RGBD)
                th = 3;
            if (mpAtlas->isImuInitialized())
    ...
    void Tracking::SearchLocalPointsAndLines()
    {
        ...
        if (nToMatch > 0)
        {
            ...
            int matchesPoints = matcher.SearchByProjection(mCurrentFrame, mvpLocalMapPoints, th, mpLocalMapper->mbFarPoints, mpLocalMapper->mThFarPoints);
        }

        nToMatch = 0;
        vector<MapLine*> mvpLocalMapLinesInFrustum;
        mvpLocalMapLinesInFrustum.reserve(mvpLocalMapLines.size());
        ...
        for(vector<MapLine*>::iterator vit = mvpLocalMapLines.begin(), vend = mvpLocalMapLines.end(); vit != vend; vit++)
        {
            ...
            {
                pML->IncreaseVisible();
                mvpLocalMapLinesInFrustum.push_back(pML);
                nToMatch++;
            }
        }
    ...
    void Tracking::UpdateLocalKeyFrames()
    {
        ...
        map<unsigned long int, KeyFrame *> keyframes;
            ...
            for (int i = 0; i < mCurrentFrame.N; i++)
            {
                ...
                if (pMP)
                {
                    ...
                    if (!pMP->isBad())
                    {
                        ...
                        {
                            keyframeCounter[observation.second.projKeyframe]++;
                            keyframes[observation.first] = observation.second.projKeyframe;
                        }
                    }
                    else
                    {
        ...
        else
        {
            for (int i = 0; i < mLastFrame.N; i++)
            {
                ...
                if (mLastFrame.mvpMapPoints[i])
                {
                    ...
                    if (mDropMoving && mLastFrame.mvKeysMoving[i])
                    {
                        MapPoint *pMP1 = mLastFrame.mvpMapPoints[i];
                        pMP1->SetBadFlag();
        ...
        {
            KeyFrame *pKF = keyframeTmp.second;

            if (pKF->isBad())
                continue;
            int keyFrameCount = keyframeCounter[pKF];
        ...
        for (vector<KeyFrame *>::const_iterator itKF = mvpLocalKeyFrames.begin(), itEndKF = mvpLocalKeyFrames.end(); itKF != itEndKF; itKF++)
        {
            ...
            const map<unsigned long int, KeyFrame *> spChilds = pKF->GetChilds();
            for (auto spChild : spChilds)
            {
                KeyFrame *pChildKF = spChild.second;
                if (!pChildKF->isBad())
                {
        ...
        if (!mpAtlas->isImuInitialized() || (mCurrentFrame.mnId < mnLastRelocFrameId + 2))
        {
            ...
            for (int i = 0; i < mCurrentFrame.N_Lines; i++)
            {
                ...
                if (pML)
                {
                    if (!pML->isBad())
                    {
                        const map<unsigned long int, ObservationLine> observations = pML->GetObservations();
                        for (const auto &observation : observations)
                        {
                            keyframeCounter[observation.second.projKeyframe]++;
                            keyframes[observation.first] = observation.second.projKeyframe;
        ...
        else
        {
            ...
            for (int i = 0; i < mLastFrame.N_Lines; i++)
            {
                ...
                if (mLastFrame.mvpMapLines[i])
                {
                    ...
                    {
                        // MODIFICATION
                        mLastFrame.mvpMapLines[i] = nullptr;
                    }
                }
            }
        }
    ...
    bool Tracking::Relocalization()
    {
        ...
        bool bMatch = false;
        ORBmatcher matcher2(0.9, true, mUseSemantic, mUseInstance);
            ...
            for (int i = 0; i < nKFs; i++)
            {
                ...
                if (bTcw)
                {
                    ...
                    for (int j = 0; j < np; j++)
                    {
                        if (vbInliers[j])
                        {
                            if (mDropMoving && mCurrentFrame.mvKeysMoving[j])
                            {
                                MapPoint *pMP1 = mCurrentFrame.mvpMapPoints[i];
                                pMP1->SetBadFlag();
    ...
    void Tracking::ResetActiveMap(bool bLocMap)
    {
        ...
        // std::cout << "First Frame id: " << index << endl;
        int num_lost = 0;
