...
namespace ORB_SLAM3
{
    ...
    LocalMapping::LocalMapping(System *pSys, Atlas *pAtlas, const float bMonocular, bool bInertial, Settings *settings, const string &_strSeqName) : mScale(1.0), mInitSect(0), mIdxInit(0), mIdxIteration(0), mbNotBA1(true), mbNotBA2(true), mpSystem(pSys), mbMonocular(bMonocular), mbInertial(bInertial),
                                                                                                                                                                                                                                      mbResetRequested(false), mbResetRequestedActiveMap(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas),
                                                                                                                                                                                                                                      mbAbortBA(false), mbStopped(false), mbStopRequested(false), mbNotStop(false), mbAcceptKeyFrames(true), bInitializing(false), infoInertial(Eigen::MatrixXd::Zero(9, 9))
    {
    ...
    void LocalMapping::newParameterLoader(Settings *settings)
    {
    ...
    void LocalMapping::RunWithLines()
    {
        mbFinished = false;
        ...
        if (CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            if (!CheckNewKeyFrames() && !stopRequested())
            {
                if (mpAtlas->KeyFramesInMap() > 2)
                {
                    ...
                    {
                        Optimizer::LocalBundleAdjustmentWithLines(mpCurrentKeyFrame, &mbAbortBA, mpCurrentKeyFrame->GetMap(), num_FixedKF_BA, num_OptKF_BA, num_MPs_BA, num_edges_BA);
                        b_doneLBA = true;
                    }
                }
#ifdef REGISTER_TIMES
    ...
    void LocalMapping::MapLineCulling()
    {
        ...
        auto lit = mlpRecentAddedMapLines.begin();
        const unsigned long int nCurrentKFid = mpCurrentKeyFrame->mnId;

        const int cnThLines = 2;

        int borrar = mlpRecentAddedMapLines.size();
    ...
    void LocalMapping::CreateNewMapPoints()
    {
        ...
        ORBmatcher matcher(th, false, mUseSemantic, mUseInstance);
        ...
        for (size_t i = 0; i < vpNeighKFs.size(); i++)
        {
            ...
            for (int ikp = 0; ikp < nmatches; ikp++)
            {
                ...
                if (mDropMoving && mpCurrentKeyFrame->mvKeysMoving[idx1])
                {
                    continue;
                }
                auto *pMP = new MapPoint(x3D, mpCurrentKeyFrame, mpAtlas->GetCurrentMap(), mpCurrentKeyFrame->mvSemanticCls[idx1], mpCurrentKeyFrame->mvInstanceCls[idx1]);
                if (bPointStereo)
                    countStereo++;
    ...
    void LocalMapping::SearchInNeighbors()
    {
        ...
        map<unsigned long int, KeyFrame*> targetKeyframes_;
        for (auto pKFi : vpNeighKFs)
        {
            if (pKFi->isBad() || targetKeyframes_.count(pKFi->mnId))
                continue;
            vpTargetKFs.push_back(pKFi);
            targetKeyframes_.insert(make_pair(pKFi->mnId,pKFi));
        }
        ...
        for (int i = 0, imax = vpTargetKFs.size(); i < imax; i++)
        {
            ...
            {
                if (pKFi2->isBad() || targetKeyframes_.count(pKFi2->mnId) || pKFi2->mnId == mpCurrentKeyFrame->mnId)
                    continue;
...
//                pKFi2->mnFuseTargetForKF = mpCurrentKeyFrame->mnId;
        ...
        for (auto pKFi : vpTargetKFs)
        {
            ...
            for (auto pMP : vpMapPointsKFi)
            {
                if (!pMP)
                    continue;
                if (pMP->isBad() || fuseCandidates_.count(pMP))
                    continue;
                vpFuseCandidates.push_back(pMP);
                fuseCandidates_.insert(pMP);
            }
        }
                ...
                if (pMP)
                {
                    if (!pMP->isBad())
                    {
                        ...
                        if (pMP->Observations() > thObs)
                        {
                            ...
                            map<KeyframeId , Observation> observations = pMP->GetObservations();
                            int nObs = 0;
            ...
            for (size_t i = 0, iend = vpMapLines.size(); i < iend; i++)
            {
                ...
                if (pML)
                {
                    if (!pML->isBad())
                    {
                        if (!mbMonocular)
                        {
                            if (pKF->mvDepthLine[i].first > pKF->mThDepth || pKF->mvDepthLine[i].second > pKF->mThDepth || pKF->mvDepthLine[i].first < 0 || pKF->mvDepthLine[i].second < 0)
                                continue;
                        }
                        ...
                        {
                            const int &scaleLevel =  pKF->mvKeysUnLine[i].octave;
                            map<KeyframeId , ObservationLine> observations = pML->GetObservations();
                            int nObs = 0;
                            for (auto & observation : observations)
                            {
                                KeyFrame* pKFi = observation.second.projKeyframe;
                                if (pKFi == pKF)
                                    continue;
                                int index = observation.second.projIndex;
                                const int &scaleLeveli = pKFi->mvKeysUnLine[index].octave;
