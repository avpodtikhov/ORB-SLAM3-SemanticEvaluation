...
namespace ORB_SLAM3
{
    LoopConnections::LoopConnections(const long unsigned int & keyframeId, KeyFrame* keyframe, const map<long unsigned int,KeyFrame*>& connections):
            keyframeId(keyframeId), keyframe(keyframe), connections(connections){
    }

    LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, const bool bActiveLC, Settings *settings) : mbResetRequested(false), mbResetActiveMapRequested(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas),
                                                                                                                                                                                                                                                                 mpKeyFrameDB(pDB), mpORBVocabulary(pVoc), mpMatchedKF(nullptr), mbLoopDetected(false), mnLoopNumCoincidences(0), mnLoopNumNotFound(0),
                                                                                                                                                                                                                                                                 ...
                                                                                                                                                                                                                                                                 mbStopGBA(false), mpThreadGBA(nullptr), mbFixScale(bFixScale), mnFullBAIdx(false), mbActiveLC(bActiveLC)
    {
    ...
    void LoopClosing::newParameterLoader(Settings *settings)
    {
        mUseSemantic = settings->useSemantic();
        mUseInstance = settings->useInstance();
        mDropDynamic = settings->dropDynamic();
        mDropMoving = settings->dropMoving();
        mUseLoop = settings->useLoop();
    ...
    {
        map<unsigned long, MapPoint *> spAlreadyMatchedMPs;
                                                 ...
                                                 int &nNumCoincidences, std::vector<MapPoint *> &vpMPs, std::vector<MapPoint *> &vpMatchedMPs)
    {
        ...
        std::map<long unsigned int, KeyFrame*> spConnectedKeyFrames = mpCurrentKF->GetConnectedKeyFrames();

        int nNumCovisibles = 10;

        ORBmatcher matcherBoW(0.9, true, mUseSemantic, mUseInstance);
        ORBmatcher matcher(0.75, true, mUseSemantic, mUseInstance);
        ...
        for (KeyFrame *pKFi : vpBowCand)
        {
            ...
            for (auto & j : vpCovKFi)
            {
                if (spConnectedKeyFrames.find(j->mnId) != spConnectedKeyFrames.end())
                {
                    bAbortByNearKF = true;
                    break;
                }
    ...
    {
        map<unsigned long, MapPoint *> spAlreadyMatchedMPs;
        for (MapPoint* mp: vpMatchedMPs) {
            spAlreadyMatchedMPs[mp->mnId] = mp;
        }
        nNumProjMatches = FindMatchesByProjection(pCurrentKF, pMatchedKF, gScw, spAlreadyMatchedMPs, vpMPs, vpMatchedMPs);
    ...
    int LoopClosing::FindMatchesByProjection(KeyFrame *pCurrentKF, KeyFrame *pMatchedKFw, g2o::Sim3 &g2oScw,
                                             map<unsigned long, MapPoint *> &spMatchedMPinOrigin, vector<MapPoint *> &vpMapPoints,
                                             vector<MapPoint *> &vpMatchedMapPoints)
    {
        ...
        map<unsigned long int, KeyFrame*> spCurrentCovisbles = pCurrentKF->GetConnectedKeyFrames();
        if (nInitialCov < nNumCovisibles)
        {
            for (int i = 0; i < nInitialCov; ++i)
            {
                ...
                {
                    if (spCheckKFs.find(vpKFs[j]) == spCheckKFs.end() && spCurrentCovisbles.find(vpKFs[j]->mnId) == spCurrentCovisbles.end())
                    {
        ...
        for (KeyFrame *pKFi : vpCovKFm)
        {
            for (MapPoint *pMPij : pKFi->GetMapPointMatches())
            {
                if (!pMPij || pMPij->isBad())
                    continue;
                if (mUseLoop && (pMPij->mvSemanticCls == 4 || pMPij->mvSemanticCls == 10))
                {
                    continue;
                }
        ...
        ORBmatcher matcher(0.9, true, mUseSemantic, mUseInstance);
    ...
    void LoopClosing::CorrectLoop()
    {
        ...
        CorrectedSim3[mpCurrentKF->mnId] = make_pair(mpCurrentKF, mg2oLoopScw);
        ...
        NonCorrectedSim3[mpCurrentKF->mnId] = make_pair(mpCurrentKF, g2oScw);
...

        {
            ...
            for (auto pKFi : mvpCurrentConnectedKFs)
            {
                if (pKFi != mpCurrentKF)
                {
                    ...
                    CorrectedSim3[pKFi->mnId] = make_pair(pKFi, g2oCorrectedSiw);
                    ...
                    NonCorrectedSim3[pKFi->mnId] = make_pair(pKFi, g2oSiw);
                }
            }
            ...
            {
                KeyFrame *pKFi = mit.second.first;
                g2o::Sim3 g2oCorrectedSiw = mit.second.second;
                g2o::Sim3 g2oCorrectedSwi = g2oCorrectedSiw.inverse();

                g2o::Sim3 g2oSiw = NonCorrectedSim3[pKFi->mnId].second;
                ...
                for (auto pMPi : vpMPsi)
                {
                    ...
                    if (pMPi->mnCorrectedByKF == mpCurrentKF->mnId)
                        continue;
                    if ((mUseLoop && (pMPi->mvSemanticCls == 4)) || (pMPi->mvSemanticCls == 10))
                    {
                        pMPi->SetBadFlag();
                        // pMPi = static_cast<MapPoint *>(nullptr);
                        continue;
                    }
...
//        map<KeyFrame *, set<KeyFrame *>> LoopConnections;
        map<unsigned long int,LoopConnections> loopConnections{};
...
//            LoopConnections[pKFi] = pKFi->GetConnectedKeyFrames();
            loopConnections.insert(std::make_pair(pKFi->mnId, LoopConnections(pKFi->mnId, pKFi, pKFi->GetConnectedKeyFrames())));

            for (auto & vpPreviousNeighbor : vpPreviousNeighbors)
            {
                loopConnections[pKFi->mnId].connections.erase(vpPreviousNeighbor->mnId);
            }
            for (auto &mvpCurrentConnectedKF: mvpCurrentConnectedKFs)
                loopConnections[pKFi->mnId].connections.erase(mvpCurrentConnectedKF->mnId);
    ...
    void LoopClosing::MergeLocal()
    {
        ...
        map<unsigned long, KeyFrame *> spLocalWindowKFs;
        // Get MPs in the welding area from the current map
        map<unsigned long, MapPoint *> spLocalWindowMPs;
        if (pCurrentMap->IsInertial() && pMergeMap->IsInertial()) // TODO Check the correct initialization
        {
            ...
            {
                spLocalWindowKFs[pKFi->mnId] = pKFi;
                pKFi = mpCurrentKF->mPrevKF;
                nInserted++;

                map<unsigned long, MapPoint *> spMPi = pKFi->GetMapPoints();
        ...
        while (spLocalWindowKFs.size() < numTemporalKFs && nNumTries < nMaxTries)
        {
            ...
            vpNewCovKFs.empty();
            for (pair<const unsigned long, KeyFrame *> pKFi : spLocalWindowKFs)
            {
                vector<KeyFrame *> vpKFiCov = pKFi.second->GetBestCovisibilityKeyFrames(numTemporalKFs / 2);
                for (KeyFrame *pKFcov : vpKFiCov)
                {
                    if (pKFcov && !pKFcov->isBad() && spLocalWindowKFs.find(pKFcov->mnId) == spLocalWindowKFs.end())
                    {
                        vpNewCovKFs.push_back(pKFcov);
                    }
                }
            }
            for (KeyFrame *pKFi : vpNewCovKFs) {
                spLocalWindowKFs[pKFi->mnId] = pKFi;
            }
            nNumTries++;
        }
        ...
        for (pair<const unsigned long, KeyFrame *> pKFi : spLocalWindowKFs)
        {
            ...
            map<unsigned long int ,MapPoint*> spMPs = pKFi.second->GetMapPoints();
        ...
        map<unsigned long, KeyFrame *> spMergeConnectedKFs;
        if (pCurrentMap->IsInertial() && pMergeMap->IsInertial()) // TODO Check the correct initialization
        {
            ...
            {
                spMergeConnectedKFs[pKFi->mnId] = pKFi;
                pKFi = mpCurrentKF->mPrevKF;
                nInserted++;
            }
        ...
        {
            spMergeConnectedKFs[mpMergeMatchedKF->mnId] = mpMergeMatchedKF;
        ...
        {
            vector<KeyFrame *> vpNewCovKFs;
            for (pair<const unsigned long, KeyFrame *> pKFi : spMergeConnectedKFs)
            {
                vector<KeyFrame *> vpKFiCov = pKFi.second->GetBestCovisibilityKeyFrames(numTemporalKFs / 2);
                for (KeyFrame *pKFcov : vpKFiCov)
                {
                    if (pKFcov && !pKFcov->isBad() && spMergeConnectedKFs.find(pKFcov->mnId) == spMergeConnectedKFs.end())
                    {
                        vpNewCovKFs.push_back(pKFcov);
                    }
                }
        ...
        map<unsigned long, MapPoint*> spMapPointMerge;
        for (pair<const unsigned long, KeyFrame *> pKFi : spMergeConnectedKFs)
        {
            map<unsigned long, MapPoint*> vpMPs = pKFi.second->GetMapPoints();
            spMapPointMerge.insert(vpMPs.begin(), vpMPs.end());
        }

        vector<MapPoint *> vpCheckFuseMapPoint;
        vpCheckFuseMapPoint.reserve(spMapPointMerge.size());
        for (pair<const unsigned long, MapPoint *> pKFi : spMapPointMerge) {
            vpCheckFuseMapPoint.push_back(pKFi.second);
        }
        ...
        vCorrectedSim3[mpCurrentKF->mnId] = make_pair(mpCurrentKF, g2oCorrectedScw);
        vNonCorrectedSim3[mpCurrentKF->mnId] = make_pair(mpCurrentKF, g2oNonCorrectedScw);
        ...
        for (pair<const unsigned long, KeyFrame *> pKFi : spLocalWindowKFs)
        {
            ...
            if (pKFi.second->GetMap() != pCurrentMap)
                Verbose::PrintMess("Other map KF, this should't happen", Verbose::VERBOSITY_DEBUG);

            g2o::Sim3 g2oCorrectedSiw;

            if (pKFi.first != mpCurrentKF->mnId)
            {
                Sophus::SE3d Tiw = (pKFi.second->GetPose()).cast<double>();
                g2o::Sim3 g2oSiw(Tiw.unit_quaternion(), Tiw.translation(), 1.0);
                // Pose without correction
                vNonCorrectedSim3[pKFi.first].second = g2oSiw;

                Sophus::SE3d Tic = Tiw * Twc;
                g2o::Sim3 g2oSic(Tic.unit_quaternion(), Tic.translation(), 1.0);
                g2oCorrectedSiw = g2oSic * mg2oMergeScw;
                vCorrectedSim3[pKFi.first] = make_pair(pKFi.second, g2oCorrectedSiw);
            }
            else
            {
                g2oCorrectedSiw = g2oCorrectedScw;
            }
            pKFi.second->mTcwMerge = pKFi.second->GetPose();

            // Update keyframe pose with corrected Sim3. First transform Sim3 to SE3 (scale translation)
            double s = g2oCorrectedSiw.scale();
            pKFi.second->mfScale = s;
            Sophus::SE3d correctedTiw(g2oCorrectedSiw.rotation(), g2oCorrectedSiw.translation() / s);

            pKFi.second->mTcwMerge = correctedTiw.cast<float>();

            if (pCurrentMap->isImuInitialized())
            {
                Eigen::Quaternionf Rcor = (g2oCorrectedSiw.rotation().inverse() * vNonCorrectedSim3[pKFi.first].second.rotation()).cast<float>();
                pKFi.second->mVwbMerge = Rcor * pKFi.second->GetVelocity();
            }
        ...
        {
            pair<const unsigned long, MapPoint *> pMPi = *itMP;
            if (!pMPi.second || pMPi.second->isBad())
            {
                itMP = spLocalWindowMPs.erase(itMP);
                continue;
            }
            if (mUseLoop && ((pMPi.second->mvSemanticCls == 4) || (pMPi.second->mvSemanticCls == 10)))
            {
                itMP = spLocalWindowMPs.erase(itMP);
                continue;
            }

            KeyFrame *pKFref = pMPi.second->GetReferenceKeyFrame();
            if (vCorrectedSim3.find(pKFref->mnId) == vCorrectedSim3.end())
            {
                itMP = spLocalWindowMPs.erase(itMP);
                numPointsWithCorrection++;
                continue;
            }
            g2o::Sim3 g2oCorrectedSwi = vCorrectedSim3[pKFref->mnId].second.inverse();
            g2o::Sim3 g2oNonCorrectedSiw = vNonCorrectedSim3[pKFref->mnId].second;

            // Project with non-corrected pose and project back with corrected pose
            Eigen::Vector3d P3Dw = pMPi.second->GetWorldPos().cast<double>();
            ...
            pMPi.second->mPosMerge = eigCorrectedP3Dw.cast<float>();
            pMPi.second->mNormalVectorMerge = Rcor.cast<float>() * pMPi.second->GetNormal();
...

        {
            ...
            for (pair<const unsigned long, KeyFrame *> pKFi : spLocalWindowKFs)
            {
                ...
                pKFi.second->mTcwBefMerge = pKFi.second->GetPose();
                pKFi.second->mTwcBefMerge = pKFi.second->GetPoseInverse();
                pKFi.second->SetPose(pKFi.second->mTcwMerge);

                // Make sure connections are updated
                pKFi.second->UpdateMap(pMergeMap);
                pKFi.second->mnMergeCorrectedForKF = mpCurrentKF->mnId;
                pMergeMap->AddKeyFrame(pKFi.second);
                pCurrentMap->EraseKeyFrame(pKFi.second);

                if (pCurrentMap->isImuInitialized())
                {
                    pKFi.second->SetVelocity(pKFi.second->mVwbMerge);
                }
            }

            for (pair<const unsigned long, MapPoint *> pMPi : spLocalWindowMPs)
            {
                if (!pMPi.second || pMPi.second->isBad())
                    continue;
                if (mUseLoop && ((pMPi.second->mvSemanticCls == 4) || (pMPi.second->mvSemanticCls == 10)))
                {
                    pCurrentMap->EraseMapPoint(pMPi.second);
                    continue;
                }
                pMPi.second->SetWorldPos(pMPi.second->mPosMerge);
                pMPi.second->SetNormalVector(pMPi.second->mNormalVectorMerge);
                pMPi.second->UpdateMap(pMergeMap);
                pMergeMap->AddMapPoint(pMPi.second);
                pCurrentMap->EraseMapPoint(pMPi.second);
            }
        ...
        for (pair<const unsigned long, KeyFrame *> pKFi : spLocalWindowKFs) {
            vpLocalCurrentWindowKFs.push_back(pKFi.second);
        }
        for (pair<const unsigned long, KeyFrame *> pKFi : spMergeConnectedKFs) {
            vpMergeConnectedKFs.push_back(pKFi.second);
        }
        if (mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO || mpTracker->mSensor == System::IMU_RGBD)
        {
        ...
        else
        {
            if (mpTracker->mSensor == System::MONOCULAR)
            {
                ...
                for (KeyFrame *pKFi : vpCurrentMapKFs)
                {
                    ...
                    g2o::Sim3 g2oSiw(Tiw.unit_quaternion(), Tiw.translation(), 1.0);
                    // Pose without correction
                    vNonCorrectedSim3[pKFi->mnId].second = g2oSiw;

                    Sophus::SE3d Tic = Tiw * Twc;
                    g2o::Sim3 g2oSim(Tic.unit_quaternion(), Tic.translation(), 1.0);
                    g2oCorrectedSiw = g2oSim * mg2oMergeScw;
                    vCorrectedSim3[pKFi->mnId] = make_pair(pKFi, g2oCorrectedSiw);
                    ...
                    if (pCurrentMap->isImuInitialized())
                    {
                        Eigen::Quaternionf Rcor = (g2oCorrectedSiw.rotation().inverse() * vNonCorrectedSim3[pKFi->mnId].second.rotation()).cast<float>();
    ...
    void LoopClosing::MergeLocal2()
    {
        ...
        {
            map<unsigned long, MapPoint *> vpMPs = pKFi->GetMapPoints();
            for (pair<const unsigned long, MapPoint *> mp : vpMPs) {
                spMapPointMerge.insert(mp.second);
            }
//            spMapPointMerge.insert(vpMPs.begin(), vpMPs.end());
    ...
    void LoopClosing::CheckObservations(map<unsigned long, KeyFrame *> &spKFsMap1, map<unsigned long, KeyFrame *> &spKFsMap2)
    {
        cout << "----------------------" << endl;
        for (pair<const unsigned long, KeyFrame *> pKFi1 : spKFsMap1)
        {
            map<unsigned long, unsigned long> mMatchedMP;
            map<unsigned long, MapPoint *> spMPs = pKFi1.second->GetMapPoints();

            for (pair<const unsigned long, MapPoint *> pMPij : spMPs)
            {
                if (!pMPij.second || pMPij.second->isBad())
                {
                    continue;
                }

                map<unsigned long, Observation> mMPijObs = pMPij.second->GetObservations();
                for (pair<const unsigned long, KeyFrame *> pKFi2 : spKFsMap2)
                {
                    if (mMPijObs.find(pKFi2.first) != mMPijObs.end())
                    {
                        if (mMatchedMP.find(pKFi2.first) != mMatchedMP.end())
                        {
                            mMatchedMP[pKFi2.first] = mMatchedMP[pKFi2.first] + 1;
                        }
                        else
                        {
                            mMatchedMP[pKFi2.first] = 1;
                        }
                    }
                }
            }
            ...
            if (mMatchedMP.empty())
            {
                cout << "CHECK-OBS: KF " << pKFi1.first << " has not any matched MP with the other map" << endl;
            }
            else
            {
                cout << "CHECK-OBS: KF " << pKFi1.first << " has matched MP with " << mMatchedMP.size() << " KF from the other map" << endl;
                for (pair<const unsigned long, unsigned long> matchedKF : mMatchedMP)
                {
                    cout << "   -KF: " << matchedKF.first << ", Number of matches: " << matchedKF.second << endl;
                }
            }
        }
        cout << "----------------------" << endl;
    ...
    {
        ORBmatcher matcher(0.8, true, mUseSemantic, mUseInstance);
        ...
        for(const auto & mit : CorrectedPosesMap)
        {
            ...
            g2o::Sim3 g2oScw = mit.second.second;
        ...
        // We need to propagate the correction through the spanning tree
        {
            ...
            if (!mbStopGBA)
            {
                ...
                while (!lpKFtoCheck.empty())
                {
                    ...
                    const map<unsigned long, KeyFrame*> sChilds = pKF->GetChilds();
                    ...
                    for (auto sChild : sChilds)
                    {
                        KeyFrame *pChild = sChild.second;
