...
namespace ORB_SLAM3
{
                                     ...
                                     int nIterations, bool *pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
    {
        ...
        for (size_t i = 0; i < vpMP.size(); i++)
        {
            ...
            const map<unsigned long int, Observation> observations = pMP->GetObservations();
            ...
            for (const auto & observation : observations)
            {
                ...
                if (leftIndex != -1 && pKF->mvuRight[get<0>(observation.second.projIndex)] < 0)
                {
                ...
                else if (leftIndex != -1 && pKF->mvuRight[leftIndex] >= 0) // Stereo observation
                {
                    ...
                    const float kp_ur = pKF->mvuRight[get<0>(observation.second.projIndex)];
                ...
                if (pKF->mpCamera2)
                {
                    int rightIndex = get<1>(observation.second.projIndex);
    ...
    void Optimizer::FullInertialBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag, bool bInit, float priorG, float priorA, Eigen::VectorXd *vSingVal, bool *bHess)
    {
        ...
        for (size_t i = 0; i < vpMPs.size(); i++)
        {
            ...
            for (const auto & observation : observations)
            {
                KeyFrame *pKFi = observation.second.projKeyframe;
                ...
                if (!pKFi->isBad())
                {
                    ...
                    if (leftIndex != -1 && pKFi->mvuRight[get<0>(observation.second.projIndex)] < 0) // Monocular observation
                    {
                        ...
                        auto *VP = dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pKFi->mnId));
                        if (bAllFixed)
                            if (!VP->fixed())
                                bAllFixed = false;
        ...
        for (auto pKFi : vpKFs)
        {
            ...
            if (pKFi->bImu)
            {
                auto *VV = dynamic_cast<VertexVelocity *>(optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 1));
                if (nLoopId == 0)
                {
                ...
                if (!bInit)
                {
                    VG = dynamic_cast<VertexGyroBias *>(optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 2));
                    VA = dynamic_cast<VertexAccBias *>(optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 3));
                }
                else
                {
                    VG = dynamic_cast<VertexGyroBias *>(optimizer.vertex(4 * maxKFid + 2));
                    VA = dynamic_cast<VertexAccBias *>(optimizer.vertex(4 * maxKFid + 3));
                }
        ...
        for (size_t i = 0; i < vpMPs.size(); i++)
        {
            ...
            MapPoint *pMP = vpMPs[i];
            auto *vPoint = dynamic_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(pMP->mnId + iniMPid + 1));
    ...
    int Optimizer::PoseOptimizationWithLines(Frame *pFrame)
    {
        ...
        // Set MapPoint vertices
        const int N = pFrame->N;
        const int N_Lines = pFrame->N_Lines;

        vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose *> vpEdgesMono;
        vector<ORB_SLAM3::EdgeLineSE3ProjectXYZOnlyPose *> vpEdgesLine;
        vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPoseToBody *> vpEdgesMono_FHR;
        vector<size_t> vnIndexEdgeMono, vnIndexEdgeRight, vnIndexEdgeLine;
        ...
        vpEdgesLine.reserve(N_Lines);
        vnIndexEdgeLine.reserve(N_Lines);
        invSigma2_Lines.reserve(N_Lines);
        ...
        unsigned int nInitialOrbEdges = 0;
        {
            unique_lock<mutex> lock(MapPoint::mGlobalMutex);
        ...
        const int thr = 50;
        int power = nInitialOrbEdges / thr;
        ...
        for (size_t it = 0; it < 4; it++)
        {
            ...
            for (size_t i = 0, iend = vpEdgesLine.size(); i < iend; i++)
            {
                ...
                if (chi2 > chi2Line)
                {
                    pFrame->mvbOutlierLine[idx] = true;
                    e->setLevel(1);
                    nBad++;
                }
                else
                {
                    e->setLevel(0);
                    pFrame->mvbOutlierLine[idx] = false;
                }
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF, int &num_MPs, int &num_edges)
    {
        ...
        map<unsigned  long int,MapPoint*> sNumObsMP;
        ...
        map<unsigned long int,KeyFrame*> fixedKeyframes;
        for (auto & lLocalMapPoint : lLocalMapPoints)
        {
            map<unsigned long int, Observation> observations = lLocalMapPoint->GetObservations();
        ...
        for (auto pMP : lLocalMapPoints)
        {
            ...
            for (const auto & observation : observations)
            {
                ...
                if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
                {
                    ...
                    // Monocular observation
                    if (leftIndex != -1 && pKFi->mvuRight[get<0>(observation.second.projIndex)] < 0)
                    {
                    ...
                    else if (leftIndex != -1 && pKFi->mvuRight[get<0>(observation.second.projIndex)] >= 0) // Stereo observation
                    {
                        const cv::KeyPoint &kpUn = pKFi->mvKeysUn[leftIndex];
                        Eigen::Matrix<double, 3, 1> obs;
    ...
    void Optimizer::LocalBundleAdjustmentWithLines(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF, int &num_MPs, int &num_edges)
    {
        // Local KeyFrames: First Breath Search from Current Keyframe
        list<KeyFrame *> lLocalKeyFrames;
        ...
        map<unsigned  long int,MapLine*> sNumObsML;
        ...
        {
            map<unsigned long int, ObservationLine> observations = lLocalMapLine->GetObservations();
            for (auto & observation : observations)
            {
                KeyFrame *pKFi = observation.second.projKeyframe;
        ...
        vpEdgeKFLine.reserve(nExpectedSizeLines);

        vector<MapLine*> vpMapLineEdge;
        vpMapLineEdge.reserve(nExpectedSizeLines);

        vector<float> Chi2Line;
        Chi2Line.reserve(nExpectedSizeLines);

        vector<float> invSigma2Line;
        invSigma2Line.reserve(nExpectedSizeLines);
        ...
        vector<int> nInitialOrbEdges(maxKFid + 1, 0);
    ...
    void Optimizer::OptimizeEssentialGraph(Map *pMap, KeyFrame *pLoopKF, KeyFrame *pCurKF,
                                           ...
                                           map<unsigned long int, LoopConnections> &LoopConnections, const bool &bFixScale)
    {
        // Setup optimizer
        g2o::SparseOptimizer optimizer;
        optimizer.setVerbose(false);
        ...
        for (auto pKF : vpKFs)
        {
            ...
            auto it = CorrectedSim3.find(pKF->mnId);
        ...
        {
            KeyFrame *pKF = LoopConnection.second.keyframe;
            const long unsigned int nIDi = pKF->mnId;
            const map<unsigned long int, KeyFrame*> &spConnections = LoopConnection.second.connections;
            const g2o::Sim3 Siw = vScw[nIDi];
            const g2o::Sim3 Swi = Siw.inverse();

            for (auto spConnection : spConnections)
            {
                const long unsigned int nIDj = spConnection.first;
                if ((nIDi != pCurKF->mnId || nIDj != pLoopKF->mnId) && pKF->GetWeight(spConnection.second) < minFeat)
                    continue;
        ...
        for (auto pKF : vpKFs)
        {
            ...
            auto iti = NonCorrectedSim3.find(pKF->mnId);
            ...
            if (pParentKF)
            {
                ...
                auto itj = NonCorrectedSim3.find(pParentKF->mnId);
            ...
            // Loop edges
            const map<unsigned long int, KeyFrame*> sLoopEdges = pKF->GetLoopEdges();
            for (auto sLoopEdge : sLoopEdges)
            {
                KeyFrame *pLKF = sLoopEdge.second;
                if (pLKF->mnId < pKF->mnId)
                {
                    g2o::Sim3 Slw;

                    auto itl = NonCorrectedSim3.find(pLKF->mnId);
            ...
            for (auto pKFn : vpConnectedKFs)
            {
                if (pKFn && pKFn != pParentKF && !pKF->hasChild(pKFn) /*&& !sLoopEdges.count(pKFn)*/)
                {
                    if (!pKFn->isBad() && pKFn->mnId < pKF->mnId)
                    {
                        ...
                        auto itn = NonCorrectedSim3.find(pKFn->mnId);
            ...
            {
                g2o::Sim3 Spw;
                auto itp = NonCorrectedSim3.find(pKF->mPrevKF->mnId);
                if (itp != NonCorrectedSim3.end())
                    Spw = itp->second.second;
                else
                                           ...
                                           vector<KeyFrame *> &vpNonFixedKFs, vector<MapPoint *> &vpNonCorrectedMPs)
    {
        ...
        map<unsigned long int, KeyFrame *> spKFs;
        ...
        for (KeyFrame *pKFi : vpKFs)
        {
            ...
            // Spanning tree edge
            if (pParentKFi && spKFs.find(pParentKFi->mnId) != spKFs.end())
            {
            ...
            // Loop edges
            const map<unsigned long int, KeyFrame*> sLoopEdges = pKFi->GetLoopEdges();
            for (auto sLoopEdge : sLoopEdges)
            {
                KeyFrame *pLKF = sLoopEdge.second;
                if (spKFs.find(pLKF->mnId) != spKFs.end() && pLKF->mnId < pKFi->mnId)
                {
                    g2o::Sim3 Slw;
                    bool bHasRelation = false;
            ...
            for (auto pKFn : vpConnectedKFs)
            {
                if (pKFn && pKFn != pParentKFi && !pKFi->hasChild(pKFn) && !sLoopEdges.count(pKFn->mnId) && spKFs.find(pKFn->mnId) != spKFs.end())
                {
    ...
    void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF, int &num_MPs, int &num_edges, bool bLarge, bool bRecInit)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            if (pKFi->bImu)
            {
                auto *VV = static_cast<VertexVelocity *>(optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 1));
                pKFi->SetVelocity(VV->estimate().cast<float>());
                auto *VG = static_cast<VertexGyroBias *>(optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 2));
                auto *VA = static_cast<VertexAccBias *>(optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 3));
                Vector6d b;
        ...
        for (auto pMP : lLocalMapPoints)
        {
            auto *vPoint = static_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(pMP->mnId + iniMPid + 1));
            pMP->SetWorldPos(vPoint->estimate().cast<float>());
            pMP->UpdateNormalAndDepth();
        }
    ...
    void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA)
    {
        ...
        // Recover optimized data
        // Biases
        VG = dynamic_cast<VertexGyroBias *>(optimizer.vertex(maxKFid * 2 + 2));
        VA = dynamic_cast<VertexAccBias *>(optimizer.vertex(maxKFid * 2 + 3));
        Vector6d vb;
        vb << VG->estimate(), VA->estimate();
        bg << VG->estimate();
        ba << VA->estimate();
        ...
        for (size_t i = 0; i < N; i++)
        {
            ...
            auto *VV = dynamic_cast<VertexVelocity *>(optimizer.vertex(maxKFid + (pKFi->mnId) + 1));
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pMainKF, vector<KeyFrame *> vpAdjustKF, const vector<KeyFrame *>& vpFixedKF, bool *pbStopFlag)
    {
        ...
        for (KeyFrame *pKFi : vpFixedKF)
        {
            ...
            map<unsigned long, MapPoint *> spViewMPs = pKFi->GetMapPoints();
            for (pair<const unsigned long, MapPoint *> pMPi : spViewMPs)
            {
                if (pMPi.second)
                    if (!pMPi.second->isBad() && pMPi.second->GetMap() == pCurrentMap)

                        if (pMPi.second->mnBALocalForMerge != pMainKF->mnId)
                        {
                            vpMPs.push_back(pMPi.second);
                            pMPi.second->mnBALocalForMerge = pMainKF->mnId;
                            numInsertedPoints++;
                        }
            }
        ...
        for (unsigned int i = 0; i < vpMPs.size(); ++i)
        {
            ...
            const map<unsigned long int, Observation> observations = pMPi->GetObservations();
            int nEdges = 0;
            // SET EDGES
            for (const auto & observation : observations)
            {
                KeyFrame *pKF = observation.second.projKeyframe;
                if (pKF->isBad() || pKF->mnId > maxKFid || pKF->mnBALocalForMerge != pMainKF->mnId || !pKF->GetMapPoint(get<0>(observation.second.projIndex)))
                    continue;

                nEdges++;

                const cv::KeyPoint &kpUn = pKF->mvKeysUn[get<0>(observation.second.projIndex)];

                if (pKF->mvuRight[get<0>(observation.second.projIndex)] < 0) // Monocular
                {
                    mpObsMPs[pMPi]++;
                    Eigen::Matrix<double, 2, 1> obs;
                    obs << kpUn.pt.x, kpUn.pt.y;
        ...
        for (auto pMPi : vpMPs)
        {
            ...
            {
                KeyFrame *pKF = observation.second.projKeyframe;
                if (pKF->isBad() || pKF->mnId > maxKFid || pKF->mnBALocalForKF != pMainKF->mnId || !pKF->GetMapPoint(get<0>(observation.second.projIndex)))
                    continue;
        ...
        for (MapPoint *pMPi : vpMPs)
        {
            ...
            auto *vPoint = dynamic_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(pMPi->mnId + maxKFid + 1));
    ...
    void Optimizer::MergeInertialBA(KeyFrame *pCurrKF, KeyFrame *pMergeKF, bool *pbStopFlag, Map *pMap, LoopClosing::KeyFrameAndPose &corrPoses)
    {
        ...
        {
            map<unsigned long int, Observation> observations = lit->first->GetObservations();
            if (i >= maxCovKF)
                break;
            for (auto & observation : observations)
            {
        ...
        for (auto pMP : lLocalMapPoints)
        {
            ...
            for (const auto & observation : observations)
            {
                ...
                if (!pKFi->isBad())
                {
                    const cv::KeyPoint &kpUn = pKFi->mvKeysUn[get<0>(observation.second.projIndex)];

                    if (pKFi->mvuRight[get<0>(observation.second.projIndex)] < 0) // Monocular observation
                    {
                        Eigen::Matrix<double, 2, 1> obs;
                        obs << kpUn.pt.x, kpUn.pt.y;
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            corrPoses[pKFi->mnId] = make_pair(pKFi, g2oSiw);

            if (pKFi->bImu)
            {
                auto *VV = dynamic_cast<VertexVelocity *>(optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 1));
                pKFi->SetVelocity(VV->estimate().cast<float>());
                auto *VG = dynamic_cast<VertexGyroBias *>(optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 2));
                auto *VA = dynamic_cast<VertexAccBias *>(optimizer.vertex(maxKFid + 3 * (pKFi->mnId) + 3));
                Vector6d b;
                b << VG->estimate(), VA->estimate();
                pKFi->SetNewBias(IMU::Bias(b[3], b[4], b[5], b[0], b[1], b[2]));
            }
                                               ...
                                               map<unsigned long int,LoopConnections> &loopConnections)
    {
        ...
        for (auto pKF : vpKFs)
        {
            ...
            {
                vScw[nIDi] = it->second.second;
                const g2o::Sim3 Swc = it->second.second.inverse();
        ...
        for(auto& mit : loopConnections)
        {
            ...
            const long unsigned int nIDi = pKF->mnId;
            const map<unsigned long int, KeyFrame*> &spConnections = mit.second.connections;
            const g2o::Sim3 Siw = vScw[nIDi];
        ...
        for (auto pKF : vpKFs)
        {
            ...
            if (prevKF)
            {
                ...
                auto itj = NonCorrectedSim3.find(prevKF->mnId);
            ...
            for (auto pKFn : vpConnectedKFs)
            {
                if (pKFn && pKFn != pParentKF && pKFn != prevKF && pKFn != pKF->mNextKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn->mnId))
                {
                    if (!pKFn->isBad() && pKFn->mnId < pKF->mnId)
                    {
                        if (sInsertedEdges.count(make_pair(min(pKF->mnId, pKFn->mnId), max(pKF->mnId, pKFn->mnId))))
