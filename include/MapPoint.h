...
namespace ORB_SLAM3
{
    ...
    public:
        ...
        tuple<unsigned long int, unsigned long int>  refIndex{};
        int refOctave{};
    ...
    public:
        ...
        MapPoint(const Eigen::Vector3f &Pos, KeyFrame *pRefKF, Map *pMap, int semantic_cls, int instance_cls);
        ...
        MapPoint(const Eigen::Vector3f &Pos, Map *pMap, Frame *pFrame, const int &idxF, int semantic_cls, int instance_cls);
        ...
        std::map<unsigned long int , Observation> GetObservations();
        int Observations();
        ...
        void increasePointObservability(KeyFrame* projKeyframe, const unsigned long int& projIndex);
        void decreasePointObservability(KeyFrame* projKeyframe, const unsigned long int& projIndex);

        void SetCurrentRefKeyframeIndex(const tuple<unsigned long int, unsigned long int>& refKeyframeIndex);
        ...
        void PreSave(std::map<long unsigned int,KeyFrame*> &spKF, std::map<long unsigned int,MapPoint*> &spMP);
        void PostLoad(map<long unsigned int, KeyFrame *> &mpKFid, map<long unsigned int, MapPoint *> &mpMPid);

    public:
        int mvSemanticCls, mvInstanceCls;
    ...
    protected:
        ...
        std::map<unsigned long int, Observation> mObservations;
        KeyFrame* currentReferenceKeyframe;
        tuple<unsigned long int, unsigned long int> currentReferenceKeypointIndex;
